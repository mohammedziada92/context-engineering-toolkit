  ...
  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-zinc-500">No data yet. Run a pipeline to see costs.</p>
      ) : (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Cost (30d)', value: formatCost(totalCost) },
                ...
                return (
                  <div key={m.model} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-zinc-400 tabular-nums w-24 text-right">
                      {formatCost(m.cost_usd)} ({m.run_count} runs)
                    </span>
                  </div>
                )
              })}
